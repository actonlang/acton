...
- `readInterfaceSessionSelection` reconstructs one exact top/member/initializer projection
...
Selective back passes first resolve every interface in the closure after the
required front passes have committed. The global worklist keeps one read
transaction open per participating interface while it reads the mandatory
summary for each selective module, whole summaries for ordinary whole
surfaces, and exact `reach/*` rows for demanded tops, shapes, members, dispatch
slots, and reflection requests. Once the closure converges,
`readInterfaceSessionSelection` reads the mandatory statement rows plus only the selected
...
selected inferred headers and merges the projected type environment. Each
materialization operation likewise keeps an ordinary read transaction open for
each participating interface.

A missing or inconsistent reachable row or an unusable selected content row is
a compiler error. An exact selection run neither widens the key nor falls back
to full module or source loading. Rootless surfaces and
...
`name-info/h/<name-hash>` value. Selective back passes build a fresh environment
from the explicit interface set resolved for the batch; lazy name and
query-index lookups use the same exact readers.
