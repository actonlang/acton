...
`$AFTER` builds only the envelope, addressed back to `self` with a future-dated
`$baseline`, and allocates no future: `after` is a statement, so its value
cannot be bound or awaited, and the dispatch loop skips result delivery for an
envelope whose `$fut` is NULL.
