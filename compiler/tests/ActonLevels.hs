...
data Var s l t  = Var t             -- A Var mentions a level l simply to connect the type methods sharing a state

data Async a b l = Async a b
...
_asyn           :: (a -> ST s l b) -> Async a b l
_snd            :: Async a b l' -> a -> ST s l (Msg b l')
await           :: Less l l' => Msg a l' -> ST s l a
...
pingpong        :: Int -> Actor (Async Int () l)
pingpong i      = actor $ do
                    count <- var 0
                    let ping q = do
                            c <- readv count
...
--session         :: (Less l_conn l_me) => Conn l_conn -> Async String () l_cb -> Actor (Async (String, Async Int () l_cb) () l_me, Async (String,String) () l_me, Async () () l_me)
...
deliver         :: Conn l -> Async String () l
close           :: Conn l -> Async () () l
receive_on      :: (Less l l1, Less l l2) => Conn l -> Async (Async String () l1, Async String () l2) () l
-- The Less constraints above make no good sense, only added for testing purposes
