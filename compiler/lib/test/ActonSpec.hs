...
withInterfaceSession path action = do
  Just snapshot <- InterfaceFiles.readModuleSnapshotMaybe path
  InterfaceFiles.withInterfaceReadSessionAtGeneration
    path (InterfaceFiles.msGeneration snapshot) action
    ...
    sequential $ describe "InterfaceFiles" $ do
        ...
        withSystemTempDirectory "acton-iface-mandatory-statements" $ \dir -> do
          ...
          withInterfaceSession tyPath $ \session ->
            InterfaceFiles.readInterfaceSessionSelection
              session [] Set.empty M.empty M.empty M.empty
              `shouldReturn` tmod
        ...
        withSystemTempDirectory "acton-iface-reachability" $ \dir -> do
          let mn = S.modName ["iface_reachability"]
              ...
              withSession action = do
                ...
                InterfaceFiles.withInterfaceReadSessionAtGeneration
                  tyPath (InterfaceFiles.msGeneration snapshot) action
      ...
      it "keeps conditional class initializer alternatives atomic" $ do
        ...
        InterfaceRows.expandMemberSelection members (Set.singleton xKey)
          `shouldBe` Right (group,Set.empty)
