  ...
  sydTest $ do
    ...
    describe "Pass 3: Types" $ do
      ...
      it "reports inferred signatures for non-total top-level statements" $ do
        ...
        _ <- liftIO $ Acton.Types.reconstruct Nothing (Just onInferred) Nothing env kchecked
    ...
    describe "Import Semantics" $ do
      it "omits private names from the public interface" $ do
        ...
        (nmodA, _, _, _) <- liftIO $ Acton.Types.reconstruct Nothing Nothing Nothing envA kcheckedA
      ...
      it "blocks qualified access to private names" $ do
        ...
        result <- liftIO $ (E.try (do
          (_, tcheckedB, _, _) <- Acton.Types.reconstruct Nothing Nothing Nothing envB kcheckedB
...
testDocGen env0 modulePaths = do
  ...
  modules <- runIO $ do
    let processModule (accEnv, accModules) modulePath = do
          ...
          (nmod, _, _, _) <- Acton.Types.reconstruct Nothing Nothing Nothing env kchecked
...
typecheckSource env0 modName src = do
  ...
  env <- liftIO $ Acton.Env.mkEnv [sysTypesPath] env0 parsed
  kchecked <- liftIO $ Acton.Kinds.check env parsed
  (_, tchecked, _, _) <- liftIO $ Acton.Types.reconstruct Nothing Nothing Nothing env kchecked
  return tchecked
...
testTypes env0 modulePaths = do
  ...
  modules <- runIO $ do
    let processModule (accEnv, accModules) modulePath = do
          (env, parsed) <- parseAct accEnv modulePath
          kchecked <- Acton.Kinds.check env parsed
          (nmod, tchecked, _, _) <- Acton.Types.reconstruct Nothing Nothing Nothing env kchecked
...
testNorm env0 modulePaths = do
  ...
  modules <- runIO $ do
    let processModule (accEnv, accModules) modulePath = do
          (env, parsed) <- parseAct accEnv modulePath
          kchecked <- Acton.Kinds.check env parsed
          (nmod, tchecked, env0Typed, _) <- Acton.Types.reconstruct Nothing Nothing Nothing env kchecked
          let I.NModule imps tenv mdoc = nmod
...
testDocstrings env0 testname = do
  ...
  kchecked <- liftIO $ Acton.Kinds.check env parsed
  (nmod, _, _, _) <- liftIO $ Acton.Types.reconstruct Nothing Nothing Nothing env kchecked
