...
instance FromJSON BuildSpec where
  parseJSON = Ae.withObject "BuildSpec" $ \o -> do
    ...
    return (BuildSpec deps zig)

instance ToJSON BuildSpec where
  toJSON (BuildSpec deps zig) = Ae.object
    [ "dependencies"     .= deps
...
renderBuildAct (BuildSpec deps zdeps) = unlines $
...
-- Extract dependencies and zig_dependencies from the AST
extractSpecFromModule :: S.Module -> BuildSpec
extractSpecFromModule (S.Module _ _ stmts) =
  let (deps, zigs) = foldl step (Map.empty, Map.empty) stmts
  in BuildSpec { dependencies = deps, zig_dependencies = zigs }
  where
    step (deps, zigs) stmt = case stmt of
      S.Assign _ pats expr ->
        let names = mapMaybe patternVarName pats
        in ( if "dependencies"     `elem` names then fromMaybe deps (exprToPkgDeps expr) else deps
           , if "zig_dependencies" `elem` names then fromMaybe zigs (exprToZigDeps expr) else zigs )
      _ -> (deps, zigs)
