...
instance FromJSON BuildSpec where
  parseJSON = Ae.withObject "BuildSpec" $ \o -> do
    ...
    return (BuildSpec nm desc fp deps zig libs)

instance ToJSON BuildSpec where
  toJSON (BuildSpec nm desc fp deps zig libs) = Ae.object $
    [ "name"            .= nm
    , "fingerprint"     .= fp
    , "dependencies"    .= deps
...
renderBuildAct (BuildSpec nm mdesc fp deps zdeps libs) = unlines $
...
-- Extract name, description, dependencies, libraries and zig_dependencies from the AST
extractSpecFromModule :: S.Module -> Either BuildSpecParseError BuildSpec
extractSpecFromModule (S.Module _ _ _ stmts) = do
  (mname, mdesc, mfp, mfpErr, deps, zigs, libs) <-
        foldM step (Nothing, Nothing, Nothing, Nothing, Map.empty, Map.empty, Map.empty) stmts
  ...
  where
    step (mname, mdesc, mfp, mfpErr, deps, zigs, libs) stmt = case stmt of
      S.Assign _ pats expr -> do
        ...
        Right (mname', mdesc', mfp', mfpErr', deps', zigs', libs')
      _ -> Right (mname, mdesc, mfp, mfpErr, deps, zigs, libs)
...
instance FromJSON BuildSpecPatch where
  parseJSON = Ae.withObject "BuildSpecPatch" $ \o -> do
    ...
    libs <- o .:? "libraries"        Ae..!= Map.empty
    pure (BuildSpecPatch name desc fp deps zigs libs)
