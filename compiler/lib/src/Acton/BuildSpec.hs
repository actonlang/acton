...
instance FromJSON BuildSpec where
  parseJSON = Ae.withObject "BuildSpec" $ \o -> do
    ...
    return (BuildSpec nm deps zig)

instance ToJSON BuildSpec where
  toJSON (BuildSpec nm deps zig) = Ae.object $
    catMaybes [ ("name" .=) <$> nm ] ++
    [ "dependencies"     .= deps
...
renderBuildAct (BuildSpec nm deps zdeps) = unlines $
...
-- Extract name, dependencies and zig_dependencies from the AST
extractSpecFromModule :: S.Module -> BuildSpec
extractSpecFromModule (S.Module _ _ stmts) =
  let (mname, deps, zigs) = foldl step (Nothing, Map.empty, Map.empty) stmts
  in BuildSpec { specName = mname, dependencies = deps, zig_dependencies = zigs }
  where
    step (mname, deps, zigs) stmt = case stmt of
