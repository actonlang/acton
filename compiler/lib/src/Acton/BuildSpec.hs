...
extractSpecFromModule (S.Module _ _ _ stmts) =
  let (mname, mdesc, mfp, mfpErr, deps, zigs, libs) =
        foldl step (Nothing, Nothing, Nothing, Nothing, Map.empty, Map.empty, Map.empty) stmts
  ...
  where
    step (mname, mdesc, mfp, mfpErr, deps, zigs, libs) stmt = case stmt of
      S.Assign _ pats expr ->
        ...
        in (mname', mdesc', mfp', mfpErr', deps', zigs', libs')
      _ -> (mname, mdesc, mfp, mfpErr, deps, zigs, libs)
