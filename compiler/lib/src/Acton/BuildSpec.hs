...
renderBuildAct (BuildSpec nm mdesc mfp deps zdeps) = unlines $
  ...
  ++ maybe [] (\fp -> ["fingerprint = " ++ renderFingerprint fp]) mfp
  ++ (if nm /= Nothing || mdesc /= Nothing || mfp /= Nothing then [""] else [])
    ...
    Ae.Object obj -> do
      ...
      let depsPresent = KM.member "dependencies" obj
          ...
          merged = BuildSpec { specName         = specName currSpec
                             ...
                             , fingerprint      = fingerprint currSpec
                             , dependencies     = deps'
...
extractSpecFromModule (S.Module _ _ stmts) =
  ...
  in BuildSpec { specName = mname, specDescription = mdesc, fingerprint = mfp, dependencies = deps, zig_dependencies = zigs }
  where
