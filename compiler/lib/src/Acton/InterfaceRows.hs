...
-- | The independently loadable syntax stored in an Acton interface file.
--
-- 'InterfaceRowsBuilder' splits a typed module into top-level statements,
-- compact container shapes, methods and attribute-initialization fragments.
-- 'InterfaceFiles' persists those rows in TYDB.  After reachability has chosen
-- exact names and members, 'SelectiveBack' asks this module to reconstruct the
-- corresponding partial Acton AST; whole-module back passes use the same
-- representation to reconstruct everything.
--
-- This module defines and reconstructs the stored syntax.  It does not decide
-- what is reachable and performs no interface-file IO.
...
-- The member map may contain only the directly read selected rows.  Expand
-- multi-name attribute groups before calling this function; no unrelated Attr
-- row needs to be enumerated.
...
expandAttrGroups members initial = do
    (selected,missing) <- expandMemberSelection members initial
    unless (S.null missing) $
...
validatePreparedContainer shape members = do
    ...
    let slots = suiteMethodSlots (shapeSuite shape)
        ...
        ordinalsByName = M.fromListWith S.union
          [ (slotName slot,S.singleton $ slotOrdinal slot) | slot <- ordinarySlots ]
    ...
    forM_ (M.toList ordinalsByName) $ \(name,ordinals) ->
      when (ordinals /= S.fromList [0 .. S.size ordinals - 1]) $
