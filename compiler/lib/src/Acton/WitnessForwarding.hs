...
-- | Resolve method-table slots implemented through another witness object.
--
-- A generated protocol witness is an ordinary Acton class whose fields may
-- contain other witnesses.  Such a class can expose a protocol method without
-- defining or inheriting that method itself: the implementation lives on a
-- witness reached through one or more of those fields.  For example, a witness
-- for one protocol may carry the witness for a parent protocol and use the
-- parent's implementation of a shared method.
--
-- CodeGen cannot put the provider's method pointer directly in the target
-- method table.  The two slots have the same explicit arguments and result,
-- but their implicit witness argument has a different concrete type.  It
-- therefore emits a small wrapper with the target slot's ABI.  The wrapper
-- starts at a constructible root witness, follows the required witness fields,
-- and calls the provider's slot with the provider object as its receiver.
--
-- The provider search originally lived in CodeGen.  Selective compilation
-- needs the same answer before CodeGen runs: when a wrapper is selected it must
-- also retain the root witness constructor, every witness field on the path,
-- the provider witness, and the provider method.  Computing that path twice
-- would allow reachability and generated C to disagree, so this module returns
-- one semantic 'ForwardPlan' consumed by both passes.  It deliberately contains
-- neither C rendering nor reachability edges.
--
-- 'buildForwardContext' indexes generated witnesses already present in the
-- current typed module.  Imported witnesses are not enumerated; they are found
-- through the exact protocol/type indexes on 'ModuleInfo'.  This distinction is
-- essential for large generated modules, where loading every imported class
-- merely to find a forwarding provider would defeat selective compilation.
--
-- For each missing method-table slot, resolution proceeds as follows:
--
-- * consider only generated witness classes and ordinary instance methods;
-- * ignore slots implemented directly or supplied by normal inheritance;
-- * find the protocol or type ancestor that owns the slot;
-- * find a constructible witness object covering that same owner;
-- * require a concrete provider method with the same ABI after removing the
--   differing witness receiver; and
-- * record the root object, field path, concrete provider and both slot types.
--
-- CodeGen renders that plan as a wrapper and installs it in the target method
-- table.  Reachability translates the same plan into dependencies.  Keeping
-- those two uses tied to one plan is the reason this logic is shared.
...
walkObject env root          = walk [] [] root
  where walk seen steps tc
          ...
          | otherwise         = ForwardObject root steps tc :
                                concat [ walk (tcname tc : seen) (steps ++ [ForwardStep tc field]) target
...
indexedWitnesses env owner   = concatMap query (importedModuleInfos env)
  ...
  where rootName'            = localQName env rootName
        ...
        follow root (Right witness : rest) object
                              = concat [ follow root rest $ ForwardObject root (objectSteps object ++ [step]) target
                                       ...
                                       , let step = ForwardStep (objectType object) (witAttr witness)
                                       ]
