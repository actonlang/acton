...
-- 'InterfaceRows'.  It also keeps every read tied to one interface generation
-- and computes the projected environments and hashes consumed by 'Compile'.
--
...
-- information for reachability closure to report a precise ambiguity instead of
...
selectFromSnapshots resolve snapshots selectableModules seeds = do
  ...
  where
    ...
    classifyOpaqueDeclarations selection = selection
      ...
      , Reach.selectedOpaqueTops =
          Reach.selectedOpaqueTops selection `Set.union` opaqueDeclarations
      }
      where
...
snapshotWholeModuleSeeds resolve snapshots modules =
    withSnapshotValidation resolve snapshots $
      withSnapshotSessions snapshots selected $ \sessions ->
