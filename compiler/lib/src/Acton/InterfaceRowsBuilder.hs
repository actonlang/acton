...
-- | Split a typed Acton module into independently loadable syntax rows.
--
-- Containers are stored as a compact lexical shape plus separate method and
-- attribute rows.  For classes, the declarative prefix of '__init__' is split
-- into attribute initialization fragments so an unused attribute can be
-- omitted without first loading its syntax.  'InterfaceFiles' writes the
-- resulting 'InterfaceRows' to TYDB, while
-- 'Reachability.prepareReachabilityRows' attaches dependency summaries to the
-- same row boundaries.
--
-- This is a front-pass syntax partitioning step.  It does not perform
-- reachability analysis, choose rows, or reconstruct syntax.
