...
readPrebuiltModuleTask paths tyFile = do
  ...
  case hdrE of
    ...
    Right (_sourceMeta, moduleSrcBytesHash, modulePubHash, moduleImplHash, imps, nameHashes, roots, tests, mdoc) ->
      let nmodStub = I.NModule [] [] mdoc
          tmodStub = A.Module mn [] mdoc []
...
enumeratePrebuiltModules ctx = do
    ...
    if not exists
      then return []
        ...
        files <- getFilesRecursive (projTypesDir ctx)
        let tyFiles = filter (\f -> takeExtension f == ".ty") files
        forM tyFiles $ \f -> do
          mn <- moduleNameFromFile (projTypesDir ctx) f
          return (f, mn)
