...
import Control.Exception (Exception, IOException, SomeException, catch, throwIO, try)
...
readModuleTask sp _gopts _opts paths actFile = do
...
runBackJobs gopts maxPar onDone jobs = loopBack indexed [] M.empty 0
  where
