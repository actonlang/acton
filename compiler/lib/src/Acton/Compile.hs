  ...
  where
    ...
    core = do
      ...
      let roots = [ n | (n,i) <- fullIface, rootEligible i ]
          ...
          moduleRows =
            case Reachability.prepareInterfaceRows rowEnv tchecked of
      ...
      where
        ...
        resolveInterface mn =
          case M.findWithDefault [] mn taskInterfaces of
            [] -> Acton.Env.findTyFile searchPaths mn
            [path] -> return (Just path)
            paths -> throwIO $ ProjectError
              ("Selective back-pass module identity is ambiguous: " ++
               show (mn,paths))
        ...
        prepareAll = do
          ...
          let deferredKeys = Data.Set.fromList (map fst deferred)
              ...
              interfaceModules = selectableModules `Data.Set.union` wholeModules0
              rootModules =
                [ (tkMod key,root)
                | (key,root) <- rootCandidates
                , M.member key taskMap
                ]
          interfaces <- SelectiveBack.loadInterfaceClosure
            resolveInterface interfaceModules
          let rootlessModules = SelectiveBack.rootlessModules interfaces
              noBackKeys
              ...
              notImplementedModules =
                SelectiveBack.notImplementedModules interfaces
          ...
          rootSeeds <- SelectiveBack.rootSeeds interfaces rootModules
          wholeSeeds <- SelectiveBack.wholeModuleSeeds interfaces interestWholeModules
          selectedPreparation <- prepareSelected interfaces selectedDeferred
          ...
          case selectedPreparation of
            Left () -> do
              ...
              wholeJobs <- prepareWholeModules interfaces deferred
              return wholeJobs
            Right (selection,selectiveJobs) -> do
              ccOnReachability callbacks interestWholeModules selection
              wholeJobs <- prepareWholeModules interfaces wholeDeferred
              return (wholeJobs ++ selectiveJobs)

        prepareSelected _ [] _ =
          return (Right (Reachability.emptySelection,[]))
        prepareSelected _ selectedDeferred [] = do
          mapM_ (ccOnBackSkipped callbacks . fst) selectedDeferred
          return (Right (Reachability.emptySelection,[]))
        prepareSelected interfaces selectedDeferred seeds = do
          let selectableModules = Data.Set.fromList
                [ dbjMod dbj | (_,dbj) <- selectedDeferred ]
          selectedResult <- SelectiveBack.selectInterfaces
            interfaces selectableModules seeds
          ...
          where
            prepareProjection selectedProgram0 = do
              (selectedProgram,opaqueHashes) <-
                SelectiveBack.selectedOpaqueHashes resolveInterface selectedProgram0
              let activeInterfaces = SelectiveBack.selectedProgramInterfaces selectedProgram
              ...
              projections <- SelectiveBack.materializeInterfaceProjections
                (Data.Set.toAscList projectionModules) selectedProgram
        ...
        prepareWholeModules interfaces wholeDeferred = do
          interfaceEnv <- SelectiveBack.interfaceEnvironment interfaces
          mapM (prepareWhole interfaces interfaceEnv) wholeDeferred

        prepareWhole interfaces interfaceEnv (key,dbj) = do
          let paths = dbjPaths dbj
              mn = dbjMod dbj
          implHash <- SelectiveBack.interfaceImplementationHash interfaces mn
          sourceHash <- SelectiveBack.interfaceSourceHash interfaces mn
          let wholeHash = Hashing.wholeCodegenHash
                (not $ C.dbg_no_lines $ dbjOpts dbj) implHash sourceHash
              codegenHash = SelectiveBack.bindInterfaces wholeHash interfaces
          status <- codegenStatus paths mn codegenHash
          if codegenUpToDate status
            then do
              ...
              unless (storedImplHash == implHash) $
                throwIO $ ProjectError $
                  ...
                  modNameToString mn
              env <- Acton.Env.mkEnv (searchPath paths) interfaceEnv typed
