  ...
  describe "selective projection" $ do
    ...
    it "distinguishes structurally different names in projection hashes" $ do
      let plain = A.name "ownerD_part"
          ...
          selected name = emptySelection
            { Reachability.selectedOpaqueTops =
    ...
    it "projects explicit imports to selected provider names" $ do
      let provider = A.modName ["provider"]
          ...
          selection = emptySelection
            { Reachability.selectedTops = Set.singleton
                (ReachRows.TopKey provider used)
            , Reachability.selectedDeclarations = Set.singleton
                (ReachRows.TopKey provider declared)
            , Reachability.selectedOpaqueTops = Set.singleton
    ...
    it "narrows wildcard enumeration without changing importability" $ do
      ...
      let provider = A.modName ["wildcard_provider"]
          ...
          selection = emptySelection
            { Reachability.selectedOpaqueTops = Set.fromList
    ...
    it "rejects a selected container without its inferred header" $ do
      withSystemTempDirectory "selective-back" $ \dir -> do
        ...
        selected <- selectFromInterfaces
          (\_ -> return [tyFile]) (Set.singleton mn) [ReachRows.Need mn name]
        program <- either (error . show) return selected
        outcome <- E.try
          (materializeInterfaceProjection (\_ -> return [tyFile]) mn program)
    ...
    it "rejects ambiguous interface providers without widening" $ do
      outcome <- E.try $ selectFromInterfaces
        (\_ -> return ["/first.tydb","/second.tydb"])
        (Set.singleton fixtureModule) [ReachRows.Need fixtureModule fixtureName]
      case outcome of
    ...
    it "classifies declarations outside deferred modules without row reads" $ do
      result <- selectFromInterfaces
        (\_ -> error "opaque declaration performed an interface read")
        Set.empty [ReachRows.Declare fixtureModule fixtureName]
      case result of
        ...
        Right program -> do
          ...
          Reachability.selectedDeclarations selection `shouldBe` Set.empty
          Reachability.selectedOpaqueTops selection `shouldBe` Set.singleton fixtureKey
    ...
    it "rejects an interface generation change during selection" $ do
      withSystemTempDirectory "selective-back" $ \dir -> do
        ...
        let resolve _ = do
              ...
              return [tyFile]
        outcome <- E.try $ selectFromInterfaces
          resolve (Set.singleton mn) [ReachRows.Need mn name]
        case outcome of
    ...
    it "rejects an interface generation change during projection" $ do
      withSystemTempDirectory "selective-back" $ \dir -> do
        ...
        write (B.pack "impl-a")
        selected <- selectFromInterfaces
          stable (Set.singleton mn) [ReachRows.Need mn name]
        program <- either (error . show) return selected
        calls <- newIORef (0 :: Int)
        let changing _ = do
              call <- atomicModifyIORef' calls $ \n -> (n + 1,n)
              ...
              return [tyFile]
        outcome <- E.try $
          materializeInterfaceProjection changing mn program
        case outcome of
          Left (Selective.ChangedInterfaceSnapshot changed path) -> do
            changed `shouldBe` mn
            path `shouldBe` tyFile
    ...
    it "rejects an interface generation change while reading opaque hashes" $ do
      withSystemTempDirectory "selective-back" $ \dir -> do
        ...
        selected <- selectFromInterfaces
          ...
          Set.empty [ReachRows.Need mn name]
        program <- either (error . show) return selected
        calls <- newIORef (0 :: Int)
        let changing _ = do
              call <- atomicModifyIORef' calls $ \n -> (n + 1,n)
    ...
    it "keeps opaque interfaces in the back-pass snapshot" $ do
      withSystemTempDirectory "selective-back" $ \dir -> do
        ...
        write (B.pack "impl-b")
        outcome <- E.try $ validateSelectedProgram stable program
        case outcome of
          Left (Selective.ChangedInterfaceSnapshot changed path) -> do
            changed `shouldBe` mn
            path `shouldBe` tyFile
...
selectFromInterfaces :: Selective.InterfaceResolver
                     -> Set.Set A.ModName
                     -> [ReachRows.ReachEdge]
                     -> IO (Either Reachability.SelectionError Selective.SelectedProgram)
selectFromInterfaces resolve modules seeds = do
    snapshots <- Selective.captureInterfaceClosure resolve modules
    Selective.selectFromSnapshots resolve snapshots modules seeds

materializeInterfaceProjection :: Selective.InterfaceResolver
                               -> A.ModName
                               -> Selective.SelectedProgram
                               -> IO Selective.Projection
materializeInterfaceProjection resolve mn program = do
    projections <- Selective.materializeInterfaceProjections resolve [mn] program
    case projections of
      [projection] -> return projection
      _ -> error "expected one projection"

validateSelectedProgram :: Selective.InterfaceResolver -> Selective.SelectedProgram -> IO ()
validateSelectedProgram resolve =
    Selective.validateInterfaceSnapshots resolve . Selective.selectedProgramSnapshots
...
emptySelection :: Reachability.Selection
emptySelection = Reachability.emptySelection
...
containerFixture = do
  ...
  reachRows <- either (error . show) return $
    Reachability.prepareReachabilityRows env tenv typed rows
