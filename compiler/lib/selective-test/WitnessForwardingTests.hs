...
tests = describe "witness slot forwarding" $ do
    it "uses a sibling witness that covers the same protocol slot" $ do
      ...
      fmap (A.tcname . Forward.forwardProviderType) (forwardPlan env context target n)
    ...
    it "does not use an unrelated class with the same concrete slot" $ do
      ...
      forwardPlan env context target n `shouldBe` Nothing
    ...
    it "reads projected imported providers through the exact witness index" $ do
      ...
      fmap (A.tcname . Forward.forwardProviderRoot) (forwardPlan env context target n)
    ...
    it "attaches forwarding provider construction to the target top" $ do
      ...
      prepared <- either (error . show) return $
        Reachability.prepareReachabilityRows env
      ...
      Set.member (ReachRows.Need (A.modname modu) provider) edges `shouldBe` True
      Set.member (ReachRows.Construct (A.modname modu) provider) edges `shouldBe` True
...
forwardPlan :: Env.Env0 -> Forward.ForwardContext -> A.TCon -> A.Name -> Maybe Forward.ForwardPlan
forwardPlan env context target name =
    find ((== name) . Forward.forwardSlotName) (Forward.forwardPlans env context target)
...
topEdges :: ReachRows.ReachabilityRows -> ReachRows.TopKey -> Set.Set ReachRows.ReachEdge
topEdges rows key = case Map.lookup key (ReachRows.reachTopRows rows) of
    Just (ReachRows.LocalTop _ summary) -> Set.fromList $ ReachRows.reachEdges summary
