      ...
      describe "persisted reachability rows" $ do
        it "keeps same-location typed receivers exact" $ do
          ...
          edgeSet summary `shouldBe` Set.fromList
            [ Reach.Declare (moduleName fixture) (aName fixture)
            , Reach.Declare (moduleName fixture) (bName fixture)
            , Reach.Dispatch (moduleName fixture) (aName fixture) (Reach.MethodRef $ runName fixture)
            , Reach.Dispatch (moduleName fixture) (bName fixture) (Reach.MethodRef $ runName fixture)
            ]
        ...
        it "terminates class-qualified calls to another member of the same class" $ do
          ...
          let mn = A.modName ["self_referencing_reach_rows"]
              ...
              lookups prepared = Reachability.ReachLookup
                { Reachability.lookupTopRow = \key ->
                    return $ Map.lookup key (Reach.reachTopRows prepared)
                , Reachability.lookupMemberRow = \owner member ->
                    return $ Map.lookup (owner,member) (Reach.reachMemberRows prepared)
                , Reachability.lookupShapeRow = \key ->
                    return $ Map.lookup key (Reach.reachShapeRows prepared)
                , Reachability.lookupSlotRow = \owner ref ->
                    return $ Map.lookup (owner,ref) (Reach.reachSlotRows prepared)
                , Reachability.lookupSurfaceSlots = \owner -> return
                    [ (ref,slot)
                    | ((owner',ref),slot) <- Map.toAscList (Reach.reachSlotRows prepared)
                    , owner' == owner
                    ]
                , Reachability.lookupReflectableAttrs = \owner ->
                    return $ Map.lookup owner (Reach.reachReflectableRows prepared)
                }
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
          selected <- Reachability.selectProgram (lookups prepared)
            [Reach.Direct mn mathUtil $ Reach.MethodRef fourth]
          result <- case selected of
              Left err -> expectationFailure ("selection failed: " ++ show err)
              Right selected -> return selected
          Reachability.selectedTops result `shouldBe` Set.fromList [mathKey,squareDepKey]
          Reachability.selectedMembers result `shouldBe` Set.fromList
        ...
        it "declares types used only by unselected method ABI slots" $ do
          ...
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
          Set.member (Reach.Declare mn hidden) (hostEdges prepared) `shouldBe` True
          Set.member (Reach.Need mn bodyOnly) (hostEdges prepared) `shouldBe` False
          Set.member (Reach.Need mn bodyOnly)
        ...
        it "keeps constructor attribute and imperative-rest summaries separate" $ do
          ...
          Set.member (Reach.Construct (moduleName fixture) $ depName fixture) attrEdges `shouldBe` True
          Set.member (Reach.Need (moduleName fixture) $ helperName fixture) attrEdges `shouldBe` False
          Set.member (Reach.Need (moduleName fixture) $ helperName fixture) restEdges `shouldBe` True
          Set.member (Reach.Construct (moduleName fixture) $ depName fixture) restEdges `shouldBe` False
        ...
        it "models captured actor parameters and visible Assign bindings as attributes" $ do
          ...
          Set.member (Reach.Construct (moduleName fixture) $ depName fixture)
            (maybe Set.empty edgeSet $ Reach.memberInstanceInitSummary cached) `shouldBe` True
          edgeSet method `shouldBe` Set.fromList
            [ Reach.Dispatch (moduleName fixture) (workerName fixture) (Reach.AttrRef $ configName fixture)
            , Reach.Dispatch (moduleName fixture) (workerName fixture) (Reach.AttrRef $ cachedName fixture)
            ]

        it "puts reflection on the getAttr member and not the container top" $ do
          let owner = hostKey fixture
              reflection = Reach.Reflect (moduleName fixture) (hostName fixture)
        ...
        it "does not turn an ordinary call into a wildcard constructor" $ do
          ...
          Set.member (Reach.Need (moduleName fixture) $ helperName fixture) rest `shouldBe` True
          Set.member (Reach.Construct (moduleName fixture) $ helperName fixture) rest `shouldBe` False
        ...
        it "records exact post-front constructor obligations without reflection" $ do
          let hostGenerated = constructorEdges rows (hostKey fixture)
              ...
              direct owner name = Reach.Direct (moduleName fixture) owner (Reach.MethodRef name)
        ...
        it "lets a concrete extension provider replace an inherited protocol signature" $ do
          let owner = hostKey fixture
              ref = Reach.MethodRef (A.name "execute")
        ...
        it "partitions class NVar bindings into exact attribute rows" $ do
          ...
          Set.member (Reach.Construct (moduleName fixture) $ depName fixture) initialized
            `shouldBe` True
          Map.lookup (owner,Reach.AttrRef attr) (Reach.reachSlotRows rows) `shouldBe`
        ...
        it "keeps a direct class attribute's conditional initializer atomic" $ do
          ...
          let mn = A.modName ["conditional_static_init_rows"]
              ...
              owner = Reach.TopKey mn cls
              directX = Reach.Direct mn cls (Reach.AttrRef x)
              need name = Reach.Need mn name
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
        ...
        it "keeps ownerless top-level dependencies in the module summary" $ do
          ...
          edgeSet (Reach.reachModuleSummary prepared) `shouldBe`
            Set.fromList
              [ Reach.Need Builtin.mBuiltin (A.name "value")
              , Reach.Construct Builtin.mBuiltin (A.name "value")
              ]
        ...
        it "persists discovered builtin keys as opaque" $ do
          ...
          Map.lookup (key,Reach.MethodRef method) (Reach.reachSlotRows prepared) `shouldBe`
        ...
        it "keeps source-backed builtin call contracts on opaque tops" $ do
          ...
          let function = A.name "next"
              ...
              expected = Reach.Dispatch Builtin.mBuiltin Builtin.nIterator
                (Reach.MethodRef Builtin.nextKW)
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
        ...
        it "uses the complete typed suite env for forward and generated classes" $ do
          ...
          edgeSet (topSummary prepared makeKey) `shouldBe` Set.fromList
            [ Reach.Declare mn className
            , Reach.Need mn className
            , Reach.Construct mn className
            ]
        ...
        it "keeps source protocol and extension kinds after backend conversion" $ do
          ...
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env source modu compact)
          ...
          maybe Set.empty edgeSet (Reach.memberStaticInitSummary classWitnessInfo)
            `shouldBe` Set.singleton (Reach.Need mn dependency)
        ...
        it "keeps an inherited class property concrete" $ do
          ...
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
          Map.lookup (childKey,Reach.AttrRef property) (Reach.reachSlotRows prepared)
            `shouldBe` Just (Reach.SlotInfo baseKey Reach.AttributeSlot)
          Set.member (Reach.AttrRef property)
            (Set.fromList $ Reach.shapeAbstracts $ shapeInfo prepared childKey)
            `shouldBe` False
          Set.member (Reach.Direct mn childName $ Reach.AttrRef property)
        ...
        it "records inherited property layout and types emitted only by whole headers" $ do
          ...
          let mn = A.modName ["whole_property_header_rows"]
              ...
              childKey = Reach.TopKey mn childName
              expectedType = Reach.Declare providerModule fieldName
              expectedLayout = Reach.DeclareAttr providerModule baseName property
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
        ...
        it "retains an ancestor property shadowed by a whole subclass" $ do
          ...
          let mn = A.modName ["whole_shadowed_property_rows"]
              ...
              expected = Reach.DeclareAttr providerModule baseName property
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
        ...
        it "records inherited class-table values required by CodeGen" $ do
          ...
          let mn = A.modName ["whole_inherited_value_rows"]
              ...
              expected = Reach.Direct mn childName (Reach.AttrRef value)
              transient = Reach.Direct mn childName (Reach.AttrRef witnessValue)
          compact <- either (error . show) return
            (RowsBuilder.prepareInterfaceRows env modu)
          prepared <- either (error . show) return
            (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
...
edgeSet :: Reach.ReachSummary -> Set.Set Reach.ReachEdge
edgeSet = Set.fromList . Reach.reachEdges

sourceInterface :: A.Module -> I.TEnv
sourceInterface = QuickType.envOfTopSuite . A.mbody

topSummary :: Reach.ReachabilityRows -> Reach.TopKey -> Reach.ReachSummary
...
memberSummary :: Reach.ReachabilityRows -> Reach.TopKey -> Rows.MemberKey -> Reach.ReachSummary
...
constructorEdges :: Reach.ReachabilityRows -> Reach.TopKey -> Set.Set Reach.ReachEdge
...
buildFixture = do
    ...
    prepared <- either (error . show) return (Reachability.prepareReachabilityRows env (sourceInterface modu) modu compact)
