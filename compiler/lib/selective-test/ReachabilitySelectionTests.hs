...
module ReachabilitySelectionTests (tests) where
  ...
  describe "reachability closure" $ do
    ...
    it "activates a local initializer for an inherited opaque attribute" $ do
      let x = A.name "field"
          ...
          baseIndex = mkIndex [cShape]
            [attrRow c x mempty (Just $ summary [need d])] [(d,mempty)]
          index = baseIndex
            { programTops = Map.insert opaque
                (Select.OpaqueTop mempty) (programTops baseIndex)
            }
    ...
    it "requests each reached row once and no unused rows" $ do
      let x = A.name "unusedAttr"
          ...
          (outcome,counts) = runState (Select.selectProgram lookups seeds) emptyCounts
      case outcome of
    ...
    it "does not request a shape for a plain Need" $ do
      let index = mkIndex [] [] [(c,mempty)]
          (outcome,counts) = runState
            (Select.selectProgram (countedLookup index) [need c]) emptyCounts
      case outcome of
        Left err -> expectationFailure ("selection failed: " ++ show err)
    ...
    it "records declarations without row reads and lets Need supersede them" $ do
      let declarationOnly = selectIndex emptyProgramIndex [declare c]
    ...
    it "rejects a selected container whose shape row is missing" $ do
      let header = I.NClass [] [] [] Nothing
          index = emptyProgramIndex
            { programTops = Map.singleton c
                (Select.LocalTop (Just header) mempty)
            }
      selectIndex index [need c] `shouldBe` Left (Select.MissingShape c)

    it "treats an opaque plain top as a terminal" $ do
      let opaque = top "Opaque"
          index = emptyProgramIndex
            { programTops = Map.singleton opaque (Select.OpaqueTop mempty) }
          (outcome,counts) = runState
            (Select.selectProgram (countedLookup index) [need opaque]) emptyCounts
      case outcome of
        Left err -> expectationFailure ("selection failed: " ++ show err)
    ...
    it "terminates an opaque slot after exact provider resolution" $ do
      let opaque = top "Opaque"
          ...
          index = index0
            { programTops = Map.insert opaque (Select.OpaqueTop mempty)
                (programTops index0) }
          (outcome,counts) = runState
            (Select.selectProgram (countedLookup index) [direct opaque ref]) emptyCounts
      case outcome of
        Left err -> expectationFailure ("selection failed: " ++ show err)
        Right _ -> return ()
      counts `shouldBe` emptyCounts
    ...
    it "terminates an inherited concrete slot at an opaque provider" $ do
      let opaque = top "OpaqueProvider"
          ...
          (outcome,counts) = runState
            (Select.selectProgram (countedLookup index) [direct c ref]) emptyCounts
    ...
    it "routes opaque construction through selectable inherited layout and methods" $ do
      let padding = A.name "_padding"
          ...
          index = index0
            { programTops = Map.insert d (Select.OpaqueTop mempty)
                (programTops index0) }
    ...
    it "retains selectable layout behind an opaque ancestor barrier" $ do
      let padding = A.name "_padding"
          ...
          index0 = mkIndex [baseShape,middleShape,childShape]
            [attrRow b padding mempty Nothing] []
          index = index0
            { programTops = Map.insert e (Select.OpaqueTop mempty)
                (programTops index0) }
    ...
    it "keeps abstract inherited slots as errors at opaque providers" $ do
      ...
      selectIndex index [direct c ref]
    ...
    it "dispatches through an opaque base using only the local subtype lineage" $ do
      let opaque = top "OpaqueBase"
          ...
          (outcome,counts) = runState
            (Select.selectProgram (countedLookup index) [dispatch opaque run,construct d])
            emptyCounts
      result <- case outcome of
        Left err -> expectationFailure ("selection failed: " ++ show err)
        Right selectedResult -> return selectedResult
    ...
    it "reports a missing exact slot" $ do
      let cShape = classShape c [c] []
          index = mkIndex [cShape] [] []
      selectIndex index [direct c (Reach.MethodRef run)]
    ...
    it "selects a witness call to its protocol backend initializer" $ do
      ...
      selectIndex index [construct protocol]
    ...
    it "terminates inherited construction at an opaque builtin shape" $ do
      let builtin = Select.TopKey Builtin.mBuiltin Builtin.nObject
          ...
          index = index0
            { programTops = Map.insert builtin (Select.OpaqueTop mempty)
                (programTops index0)
            }
      result <- selected index [construct d]
...
data ProgramIndex = ProgramIndex
  { programTops             :: Map.Map Select.TopKey Select.TopInfo
  , programMembers          :: Map.Map (Select.TopKey, Rows.MemberKey) Select.MemberInfo
  , programShapes           :: Map.Map Select.TopKey Select.ShapeInfo
  , programSlots            :: Map.Map (Select.TopKey, Reach.MemberRef) Select.SlotInfo
  , programReflectableAttrs :: Map.Map Select.TopKey Select.ReflectableAttrs
  }
...
selectIndex :: ProgramIndex -> [Reach.ReachEdge] -> Either Select.SelectionError Select.Selection
selectIndex index seeds = runIdentity $ Select.selectProgram (indexLookup index) seeds

indexLookup :: Applicative m => ProgramIndex -> Select.ReachLookup m
indexLookup index = Select.ReachLookup
  { Select.lookupTopRow = \key -> pure $ Map.lookup key (programTops index)
  , Select.lookupMemberRow = \owner member ->
      pure $ Map.lookup (owner,member) (programMembers index)
  , Select.lookupShapeRow = \key -> pure $ Map.lookup key (programShapes index)
  , Select.lookupSlotRow = \owner ref ->
      pure $ Map.lookup (owner,ref) (programSlots index)
  , Select.lookupSurfaceSlots = \owner -> pure
      [ (ref,slot)
      | ((owner',ref),slot) <- Map.toAscList (programSlots index)
      , owner' == owner
      ]
  , Select.lookupReflectableAttrs = \owner ->
      pure $ Map.lookup owner (programReflectableAttrs index)
  }


methodHierarchy :: ProgramIndex
...
countedLookup index = Select.ReachLookup
  ...
  , Select.lookupReflectableAttrs = \receiver -> do
      ...
      return $ Map.lookup receiver (programReflectableAttrs index)
  }
  where bump = modify'
...
mkIndex :: [ShapeFixture]
        ...
        -> ProgramIndex
mkIndex shapes givenMembers plainTops = ProgramIndex
  { programTops = Map.fromList
...
selected :: ProgramIndex -> [Reach.ReachEdge] -> IO Select.Selection
selected index seeds =
  case selectIndex index seeds of
    Left err -> expectationFailure ("selection failed: " ++ show err)
