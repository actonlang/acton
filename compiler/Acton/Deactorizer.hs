...
deactA env (Actor l n q p k t b)    = do n' <- newName (nstr n)
                                         ...
                                         let (ssigs,sext') = partition isSig sext
                                             ...
                                             create = Update l0 [selfPat selfKW] (Call l0 (Var l0 (NoQName n')) (parToArg p) KwdNil)
                                             copies = [ Update l0 [selfPat n] (Dot l0 (Dot l0 (Var l0 (NoQName selfKW)) selfKW) n) | n <- consts ]
  ...
  where env1 n'                     = env{ locals = nub $ bound (p,k) ++ bound b ++ statedefs b, actor = Just n' }
        selfPat n                   = TaDot l0 (Var l0 (NoQName selfKW)) n
...
parToArg (PosPar n _ _ p)           = PosArg (Var l0 (NoQName n)) (parToArg p)

asyncCall env n p                   = Call l0 (Var l0 primASYNC) (PosArg selfSelf (PosArg clos PosNil)) KwdNil
  where selfSelf                    = Dot l0 (Var l0 (NoQName selfKW)) selfKW
...
instance Deact Expr where
    deact env (Var l (NoQName n))
      | selfRef n env               = return $ Dot l (Var l (NoQName selfKW)) n
...
instance Deact Target where
    deact env (TaVar l n)
      | selfRef n env               = return $ TaDot l (Var l (NoQName selfKW)) n
