...
xmlQ_Node $NodePtr2Node(xmlNodePtr node, B_str *error) {
    ...
    // tail.
    while (cur != NULL) {
        xmlQ_Node child = $NodePtr2Node(cur, error);
        if (child == NULL)
            return NULL;
        cur = cur->next;
...
xmlQ_Node xmlQ_decode(B_str data) {
    ...
    B_str error = NULL;
    xmlQ_Node t = $NodePtr2Node(root, &error);
    xmlFreeDoc(doc);
    if (t == NULL)
        RAISE(xmlQ_XmlParseError, error, NULL, NULL);
    return t;
}
