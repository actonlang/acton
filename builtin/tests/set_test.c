...
int main() {
  $Set$set wit= $Set$set_new(($Hashable)$Hashable$int$witness);
  ...
  $set s4 = wit->_Logical->$class->__or__(wit->_Logical,s,s2);
  printf("size of union is %ld (should be 1458)\n",from$int(wit->$class->__len__(wit,s4)));
  printf("checking if union is gt both operands; returns %ld and %ld\n",from$bool(wit->_Ord->$class->__gt__(wit->_Ord,s4,s)),from$bool(wit->_Ord->$class->__gt__(wit->_Ord,s4,s2)));
  $set s5 = wit->_Logical->$class->__xor__(wit->_Logical,s,s2);
