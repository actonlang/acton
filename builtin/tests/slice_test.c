...
int main() {
  ...
  $list lst = wit->_Collection->class->__fromiter__( wit->_Collection,NULL);
  for (long i=0; i<100; i++)
  ...
  $list lst3 = wit->_Collection->class->__fromiter__( wit->_Collection,NULL);
  for (long i=100; i<110; i++)
  ...
  stop = 30;
  step = 2;
  $Iterable$opaque it = $Iterable$pack(($Iterable)wit->_Collection,lst3);
